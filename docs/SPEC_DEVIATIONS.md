...
|---|---|---|---|
| 1 | Annex A (ADR-002), section 5.2, section 17 | Web Serial/RFCOMM is the primary Android transport; BLE/GATT is an optional, experimental fallback | Inverted by vehicle evidence. `AndroidBleObdTransport` is the only transport validated against the real car (2026-08-24, Kia Rio YB 2019 1.2 MPI). `WebSerialRfcommTransport` exists and has unit tests against a fake port, but was never run against the vehicle. See `docs/decisions/ADR-002-obd-transport.md` for the full evidence trail. |
