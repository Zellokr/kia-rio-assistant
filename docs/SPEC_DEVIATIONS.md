...
|---|---|---|---|
| 1 | Annex A (ADR-002), section 5.2, section 17 | Web Serial/RFCOMM is the primary Android transport; BLE/GATT is an optional, experimental fallback | Inverted by vehicle evidence, then removed outright on 2026-08-25. `AndroidBleObdTransport` is the only transport implementation in the repository, and the only one validated against the real car. Evidence started with the 2026-08-24 Kia Rio session over BLE, then the 2026-08-28 field test added eleven consecutive ready connections, Android WebView IndexedDB persistence, two detected drops, and one observed recovery. A2 still has one waived gap: the post-fix Bluetooth-toggle recovery path has not been rerun on the vehicle. `WebSerialRfcommTransport` had unit tests against a fake port but was never run against the vehicle, and was deleted because official Chrome documentation limits native Web Serial to desktop platforms — it could never execute inside the Capacitor Android app — and because the owned VEEPEAK adapter is BLE, a different protocol from RFCOMM. See the amendment in `docs/decisions/ADR-002-obd-transport.md` and ADR-004 for the full evidence trail. |
